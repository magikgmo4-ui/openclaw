...
- `README-OPENCLAW` - `DONE` - `README.md` existe et sert de point d entree rapide du repo dedie.
- `HERMES-DOC-BLOCK` - `DONE` - bloc Hermes documentaire initial produit avec positionnement, spec, exemple YAML, classification et closeout.

## 4. A OUVRIR

- `REVALIDATION-REPO-FINALE` - `A OUVRIR` - a lancer maintenant que `README.md` est present et que le bloc Hermes documentaire initial est clos ; doit rendre le verdict final sur la creation du repo GitHub dedie.

Prochain pas logique immediat : lancer une revalidation documentaire finale courte.
...
- `HERMES-MICROCHANTIER-STUDENT` - `FUTUR` - qualification runtime Hermes bornee sur `student` seulement si un GO explicite l ouvre.
